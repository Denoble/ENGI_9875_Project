  ...
  int socket) /* Socket from which the file descriptor is read */
{
 ...
 int main(int argc,char* argv[]){
	...
	cap_rights_init(&rights,CAP_FSTAT,CAP_READ);//initializing cap rights
	cap_rights_init(&wrights,CAP_FSTAT,CAP_WRITE,CAP_CREATE);// initializing cap rights
	...
	if(result== -1){
      ...
      //cap_enter();// enables capability mode
	...
	printf("Time cost with CAP_ENTER enabled in nano seconds is printed in execution_cost_a.txt file\n");
	pfd=open("execution_cost_a.txt",O_WRONLY|O_CREAT|O_APPEND);
	if(pfd<0){
